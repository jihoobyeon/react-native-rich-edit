...
struct HeadlessJsTaskSupportSpec : winrt::Microsoft::ReactNative::TurboModuleSpec {
  static constexpr auto methods = std::tuple{
      ...
      Method<void(double, Promise<bool>) noexcept>{1, L"notifyTaskRetry"},
  };
  ...
  static constexpr void ValidateModule() noexcept {
    ...
    REACT_SHOW_METHOD_SPEC_ERRORS(
          1,
          "notifyTaskRetry",
          "    REACT_METHOD(notifyTaskRetry) void notifyTaskRetry(double taskId, ::React::ReactPromise<bool> &&result) noexcept { /* implementation */ }\n"
          "    REACT_METHOD(notifyTaskRetry) static void notifyTaskRetry(double taskId, ::React::ReactPromise<bool> &&result) noexcept { /* implementation */ }\n");
  }
};
